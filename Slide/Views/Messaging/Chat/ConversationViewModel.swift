...
class ConversationViewModel: ObservableObject {
    ...
    var users: [UserData] // not including current user
    ...
    init(conversation: Conversation) {
        self.users = conversation.users.filter { $0.userID != SessionManager.currentUserId }
                ...
                let messages = snapshot?.documents
                    ...
                    .compactMap { firestoreMessage -> Message? in
                        guard
                            ...
                            let user = self.allUsers.first(where: { $0.userID == firestoreMessage.userId }),
                        ...
                        if let reply = firestoreMessage.replyMessage,
                           ...
                           let replyUser = self.allUsers.first(where: { $0.userID == reply.userId }) {
                            replyMessage = ReplyMessage(
                                id: replyId,
                                user: ExyteChat.User(id: replyUser.userID, name: replyUser.username, avatarURL: URL(string: replyUser.photoURL), isCurrentUser: false),
                                text: reply.text,
                        ...
                        return Message(
                            id: id,
                            user:  ExyteChat.User(id: user.userID, name: user.username, avatarURL: URL(string: user.photoURL), isCurrentUser: false),
                            status: .sent,
                            createdAt: date,
    ...
    func sendMessage(_ draft: DraftMessage) {
        Task {
            ...
            let message = await Message.makeMessage(id: id, user: ExyteChat.User(id: user.userID, name: user.username, avatarURL: URL(string: user.photoURL), isCurrentUser: true), status: .sending, draft: draft)
            lock.withLock {
    ...
    func subscribeToConversationCreation(user: UserData) {
    ...
    private func conversationForUser(_ user: UserData) -> Conversation? {
    ...
    private func createIndividualConversation(_ user: UserData) async -> Conversation? {
        subscribtionToConversationCreation = nil
        let allUserIds = allUsers.map { $0.userID }
        let dict: [String : Any] = [
            "users": allUserIds,
